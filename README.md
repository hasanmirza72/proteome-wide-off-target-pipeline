...
The pipeline is benchmarked against two established sequence- and structure-similarity methods, BLAST+ and Foldseek. Splitting the 26 documented off-targets into three structural tiers shows the methods are **complementary rather than competitive.** On pocket-similar off-targets (11 of 26), all three methods recover 10 at comparable median ranks — FoldDisco does not dominate here, but it recovers a partly different set (it alone reaches crizotinib's AURKA, and ranks several targets higher than either baseline). The entire recall difference lies in the second tier, **fold-related but pocket-dissimilar off-targets** imatinib's kinase targets, where sequence and global-structure search recover all four but pocket geometry recovers none, because a shared fold need not mean a shared pocket. In the third tier, **fold-unrelated off-targets**, all three methods recover zero: a boundary set by the off-target biology, not by any one method, since there is no similarity of any kind to detect. FoldDisco is therefore best read as a precision-oriented addition to sequence and structure search, strongest where the off-target shares the query's pocket, and not a replacement for them.
